...
5. **Color comes only from Golden Default tokens.** Every visible color must trace back to the palette below.
   *Effect domain content exception (T-015)*: User-selected effect colors are domain content, not UI chrome. Therefore arbitrary RGB values are allowed only for: effect color swatches, current-color previews, rendered Trail effects, rendered Click effects, and future explicit effect previews. They must NOT recolor window backgrounds, tabs, ordinary buttons, borders, labels, selection states, focus states, or status/error UI. All surrounding chrome remains Golden Default. Do not add a second application theme.
