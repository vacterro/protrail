...
if not exist build\CMakeCache.txt (
    echo [build_t016] ERROR: build directory not configured. Run cmake configure first. 1>&2
    exit /b 1
)

cmake --build build --config Release -- -m 2>&1
if errorlevel 1 (
    echo [build_t016] FAILED: cmake build returned errorlevel %ERRORLEVEL% 1>&2
    exit /b 1
)
echo [build_t016] Release build succeeded.
exit /b 0
